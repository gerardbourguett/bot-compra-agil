...
async def analizar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Estrategia de precio (sin mostrar precio específico)
    ...
    if precio.get('estrategia'):
        ...
        if precio.get('rango_minimo') and precio.get('rango_maximo'):
            mensaje += f"📊 <b>Rango de Mercado:</b> ${precio.get('rango_minimo', 0):,} - ${precio.get('rango_maximo', 0):,} CLP\n"
            mensaje += f"<i>⚠️ Estos son valores referenciales. Cotiza según tus costos reales.</i>\n\n"
    
    mensaje += f"📋 <b>Código:</b> <code>{codigo}</code> (toca para copiar)\n\n"
    mensaje += f"🔗 Más detalles: /ayuda_cotizar {codigo}\n"
...
# Continúa en bot_inteligente_parte3.py...
